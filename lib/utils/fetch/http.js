    ...
    switch (status) {
        ...
        default:
            if (onError) {
                ...
                _index.Toast.info(typeof data.message === 'string' ? data.message : '亲，服务器开了会小差，马上回来');
                reject(data);
            }
            break;
    }
...
function handleError(error) {
    switch (error) {
        case 401:
            ...
            break;
        case 404:
            _index.Toast.info('404');
            break;
        default:
            _index.Toast.info(error);
            break;
    }
}
...
function isNetwork() {
    _native2.default.networkStatus(function (status) {
        if (status === 0) {
            _index.Toast.offline('网络不给力，请检查网络');
        }
    });
}
